...
4. **Delegate.** Post to `mission-smoke`: "lead-a: have worker-a-1 write the file
   `smoke-ok.txt` containing OK, then confirm." agent-comms messages land in an
   idle Claude agent's input **unsent**, so wake the target after posting:
   `python3 orchestration/spawn.py poke smoke lead-a` (and, if the worker went
   idle before the task was posted, `... poke smoke worker-a-1`). **PASS** when
   `smoke-ok.txt` exists in the repo and the confirmation lands in `squad-lead-a`.
   Confirm the orchestrator is **not** a member of `squad-lead-a` (hierarchy holds).
5. **Layer guard.** `python3 orchestration/spawn.py selfcheck` prints
   `selfcheck ok`. **PASS** on exit 0.
6. **Teardown.** `python3 orchestration/spawn.py down smoke` (this pokes each lead
   to destroy its own `squad-<lead>` room before closing the workspace), then
   agent-comms `destroy_room mission-smoke` (orchestrator owns that one). **PASS**
   when `herdr workspace list` and `list_rooms` no longer show `mission-smoke` or
   `squad-lead-a`. Delete `smoke-ok.txt`.
...
`orchestration/spawn.py`. If an agent sits idle with a prompt typed but unsent,
that is the comms-nudge-not-submitted issue - `poke` it.
